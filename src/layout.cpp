...
{
    for (auto &ws : workspaces) {
        if (ws.fullscreenWindow == toplevel)
            return &ws;
        if (ws.maximizedWindow == toplevel)
            return &ws;
        if (findLeaf(ws.root.get()
            , toplevel)) return &ws;
...
LayoutManager::BspNode* LayoutManager::findLeaf(BspNode *root, Toplevel *tl) const
{
    ...
    if (auto *l = findLeaf(root->left.get()
        , tl)) return l;
    if (auto *r = findLeaf(root->right.get()
        , tl)) return r;
    return nullptr;
}
...
LayoutManager::BspNode* LayoutManager::findLeafWithParent(BspNode *root, Toplevel *tl, BspNode **outParent) const
{
    ...
    if (auto *l = findLeafWithParent(root->left.get()
        , tl, outParent)) return l;
    if (auto *r = findLeafWithParent(root->right.get()
        , tl, outParent)) return r;
    return nullptr;
}
...
LayoutManager::BspNode* LayoutManager::findParentOfLeaf(BspNode *root, Toplevel *tl) const
{
    ...
    std::function<BspNode*(BspNode*, BspNode*)> dfs = [&](BspNode* node, BspNode* /*par*/) -> BspNode* {
        ...
        if (auto *a = dfs(node->left.get()
            , node)) return a;
        if (auto *b = dfs(node->right.get()
            , node)) return b;
        return nullptr;
    };
...
if (fw.width > usable.width) {
                fw.width = usable.width;
                needsApply = true;
            }
if (fw.height > usable.height) {
                fw.height = usable.height;
                needsApply = true;
            }
            int nx = fw.x;
            int ny = fw.y;
            ...
            if (ny < usable.y)
                ny = usable.y;
            if (nx + fw.width > usable.x + usable.width)
                nx = usable.x + usable.width - fw.width;
            if (ny + fw.height > usable.y + usable.height)
                ny = usable.y + usable.height - fw.height;
if (nx != fw.x || ny != fw.y) {
                fw.x = nx;
                fw.y = ny;
                needsApply = true;
...
if (node == target) {
    ...
    if (node->orientation == Orientation::Horizontal) {
        ...
        struct wlr_box lb = { box.x, box.y, lw, box.height };
        struct wlr_box rb = { box.x + lw, box.y, rw, box.height };
        if (getBoxForNodeRecursive(node->left.get()
            , target, lb, out)) return true;
        if (getBoxForNodeRecursive(node->right.get()
            , target, rb, out)) return true;
    } else {
        int th = (int)(box.height * r);
        int bh = box.height - th;
        struct wlr_box tb = { box.x, box.y, box.width, th };
        struct wlr_box bb = { box.x, box.y + th, box.width, bh };
        if (getBoxForNodeRecursive(node->left.get()
            , target, tb, out)) return true;
        if (getBoxForNodeRecursive(node->right.get()
            , target, bb, out)) return true;
    }
    return false;
}
...
namespace {
bool containsBspNode(LayoutManager::BspNode* root, LayoutManager::BspNode* target) {
    if (!root) return false;
    if (root == target) return true;
    if (root->type == LayoutManager::BspNode::Type::Branch) {
        return containsBspNode(root->left.get(), target) || containsBspNode(root->right.get(), target);
    }
    return false;
}
} // namespace
...
bool LayoutManager::replaceNode(std::unique_ptr<BspNode> &root, BspNode *target, std::unique_ptr<BspNode> replacement)
{
    ...
    if (root->type == BspNode::Type::Branch) {
        if (containsBspNode(root->left.get(), target)) {
            return replaceNode(root->left, target, std::move(replacement));
        }
        if (containsBspNode(root->right.get(), target)) {
            return replaceNode(root->right, target, std::move(replacement));
        }
    }
    return false;
}
...
void LayoutManager::addWindow(Toplevel *toplevel, int workspace, Toplevel *focused, struct wlr_box usable, double cursorX, double cursorY)
{
    ...
    std::function<bool(std::unique_ptr<BspNode>&)> doSplit = [&](std::unique_ptr<BspNode> &node) -> bool {
        ...
        if (node->type == BspNode::Type::Branch) {
            if (doSplit(node->left)
                ) return true;
            if (doSplit(node->right)
                ) return true;
        }
        return false;
    };
...
void LayoutManager::insertWindowAt(Toplevel *toplevel, int workspace, int index)
{
    ...
    if (index < 0)
        index = 0;
    if (index >= (int)
        leaves.size()) index = leaves.size() - 1;
...
void LayoutManager::insertWindowAtCursor(Toplevel *toplevel, int workspace, struct wlr_box usable, double cursorX, double cursorY, double ratioHint)
{
    ...
    if (parent) {
        if (oppositeOrientation)
            newOrient = opposite(parent->orientation);
...
void LayoutManager::arrange(struct wlr_box usable, struct wlr_box full, int workspace)
{
    ...
    if (!ws)
        return;
    if (!ws->root && ws->floating.empty()
        && !ws->fullscreenWindow && !ws->maximizedWindow) return;
...
if (leaf->width > usable.width) {
                leaf->width = usable.width;
                needsApply = true;
            }
if (leaf->height > usable.height) {
            ...
            if (ny < usable.y)
                ny = usable.y;
            if (nx + leaf->width > usable.x + usable.width)
                nx = usable.x + usable.width - leaf->width;
            if (ny + leaf->height > usable.y + usable.height)
                ny = usable.y + usable.height - leaf->height;
if (nx != leaf->x || ny != leaf->y) {
                leaf->x = nx;
                leaf->y = ny;
                needsApply = true;
            }
...
bool LayoutManager::setFloating(Toplevel *toplevel, bool makeFloating, struct wlr_box usable)
{
    ...
    if (makeFloating == currentlyFloating)
        return false;
    ...
    if (makeFloating) {
        ...
        if (!leaf)
            return false;
            // not in tiled tree, maybe already floating?;
        int fx = leaf->x;
        int fy = leaf->y;
        int fw = leaf->width;
        int fh = leaf->height;
...
bool LayoutManager::setMaximized(Toplevel *toplevel, bool maximized)
{
    ...
    if (maximized) {
        if (ws->fullscreenWindow == toplevel)
            return false;
            // cannot maximize a fullscreen window;
        if (ws->fullscreenWindow)
            return false;
            // fullscreen takes precedence, refuse maximize while fullscreen active;
...
void LayoutManager::arrangeOthers(const struct wlr_box &usable, int workspace, Toplevel *excluded)
{
    ...
    if (ws->maximizedWindow) {
        ...
        if (ws->maximizedWindow == excluded)
            return;
    ...
    if (ws->mode == Mode::Floating) {
        ...
        for (auto *leaf : leaves) {
            ...
            if (leaf->y < usable.y)
                leaf->y = usable.y;
            if (leaf->x + leaf->width > usable.x + usable.width)
                leaf->x = usable.x + usable.width - leaf->width;
            if (leaf->y + leaf->height > usable.y + usable.height)
                leaf->y = usable.y + usable.height - leaf->height;
            applyWindowGeometry(leaf);
        }
        for (auto &fw : ws->floating) {
            if (fw.toplevel == excluded)
            ...
            if (fw.y < usable.y)
                fw.y = usable.y;
            if (fw.x + fw.width > usable.x + usable.width)
                fw.x = usable.x + usable.width - fw.width;
            if (fw.y + fw.height > usable.y + usable.height)
                fw.y = usable.y + usable.height - fw.height;
            applyFloatingGeometry(fw);
        }
        return;
    }
    ...
    if (ws->root)
        arrangeNodeOthers(ws->root.get(), usable, excluded);
...
{
    Workspace *ws = findWorkspace(workspace);
    if (!ws || !ws->root || !focused)
        return false;
    if (ws->fullscreenWindow == focused || ws->maximizedWindow == focused)
        return false;
...
{
    if (!ancestor || ancestor->type != LayoutManager::BspNode::Type::Branch)
        return false;
    ...
    std::function<bool(LayoutManager::BspNode*)> dfs = [&](LayoutManager::BspNode *node) -> bool {
        ...
        if (node->type == LayoutManager::BspNode::Type::Branch) {
            if (dfs(node->left.get()
                )) return true;
            if (dfs(node->right.get()
                )) return true;
        }
        return false;
    };
    ...
    for (auto &ws : workspaces) {
        ...
        auto tryHandle = [&](Orientation orient) -> bool {
            for (BspNode *anc = leaf->parent; anc != nullptr; anc = anc->parent) {
                ...
                if (!isLeftOrTop && !isRightOrBottom)
                    continue;
                if (orient == Orientation::Horizontal) {
                    if (isLeftOrTop && !(edges & WLR_EDGE_RIGHT)
                        && edges != 0) continue;
                    if (isRightOrBottom && !(edges & WLR_EDGE_LEFT)
                        && edges != 0) continue;
                    ...
                    if (isLeftOrTop && !(edges & WLR_EDGE_BOTTOM)
                        && edges != 0) continue;
                    if (isRightOrBottom && !(edges & WLR_EDGE_TOP)
                        && edges != 0) continue;
                    struct wlr_box ancBox = getBoxForNode(ws.root.get(), anc, usable);
                    if (ancBox.height <= 0)
                        continue;
        ...
        if (needH && needV) {
            ...
            if (tryHandle(Orientation::Horizontal)
                ) return true;
        } else if (needV) {
            if (tryHandle(Orientation::Vertical)
                ) return true;
        } else {
            if (tryHandle(Orientation::Horizontal)
                ) return true;
...
bool LayoutManager::commitResize(Toplevel *toplevel, struct wlr_box usable, uint32_t edges)
{
    for (auto &ws : workspaces) {
        ...
        if (needH && needV) {
            ...
            if (tryCommit(Orientation::Horizontal)
                ) return true;
        } else if (needV) {
            if (tryCommit(Orientation::Vertical)
                ) return true;
        } else {
            if (tryCommit(Orientation::Horizontal)
                ) return true;
...
bool LayoutManager::getWindowGeometry(Toplevel *toplevel, int workspace, struct wlr_box usable, struct wlr_box &out) const {
    const Workspace *ws = findWorkspace(workspace);
    if (!ws)
        return getWindowGeometry(toplevel, out);
    if (auto *fw = findFloating(ws, toplevel)) {
        out = {fw->x, fw->y, fw->width, fw->height};
        return true;
    }
...
std::unordered_map<Toplevel*, struct wlr_box> LayoutManager::snapshotGeometries(int workspace, struct wlr_box usable) const {
    ...
    for(auto &p: boxes){
        if(p.first->toplevel)
            out.emplace(p.first->toplevel, p.second);
    }
    for(auto &fw: ws->floating){
        if(fw.toplevel){
            struct wlr_box b = {fw.x, fw.y, fw.width, fw.height};
