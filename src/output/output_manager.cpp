...
void OutputManager::onOutput(const VariantEvent& ev) {
    auto* o = std::get_if<OutputEvent>(&ev);
    if (!o || !o->hasEvent)
        return;
    OutputInfo info;
    ...
    outputs.insert_or_assign(info.serial, info);
}
...
OutputInfo OutputManager::infoFor(uint32_t serial) const noexcept {
    ...
    if (it != outputs.end()
        ...
        // copy small struct (32 bytes) — acceptable;
    return OutputInfo{};
}
...
if (anchor & TOP) {
        usable.y += zone + margin.y;
        usable.height -= zone + margin.y;
    }
else if (anchor & BOTTOM) {
        usable.height -= zone + margin.height;
    }
if (anchor & LEFT) {
        usable.x += zone + margin.x;
        usable.width -= zone + margin.x;
    }
else if (anchor & RIGHT) {
        usable.width -= zone + margin.width;
    }
    if (usable.width < 0)
        usable.width = 0;
    if (usable.height < 0)
