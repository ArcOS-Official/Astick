...
{
    wlr_log(WLR_INFO, "Compositor::onToplevelUnmapped tl %p id %lu", (void*)toplevel, toplevel ? (unsigned long)toplevel->id : 0);
...
void Compositor::startCloseAnimation(Toplevel *toplevel)
{
    if (!toplevel) return;
    wlr_log(WLR_INFO, "Compositor::startCloseAnimation tl %p id %lu", (void*)toplevel, (unsigned long)toplevel->id);
    dumpDebugState();
    ...
    : config(cfg)
{
    ...
    connect(animPool, &AnimationPool::frameRequested, this, &Compositor::scheduleAllOutputs);
    ...
    connect(animManager, &AnimationManager::frameRequested, this, &Compositor::scheduleAllOutputs);
...
void Compositor::scheduleAllOutputs() {
...
struct wlr_box Compositor::boxForStyle(AnimationStyle style, const struct wlr_box &finalBox, bool isStart) {
...
void Compositor::animateBoxForToplevel(Toplevel *tl, struct wlr_box from, struct wlr_box to, const AnimDef &def, const QString &id) {
...
void Compositor::animateTilingMove(const std::unordered_map<Toplevel*, struct wlr_box> &before, const std::unordered_map<Toplevel*, struct wlr_box> &after) {
...
void Compositor::animateWindowOpen(Toplevel *tl, const struct wlr_box &finalBox) {
...
void Compositor::animateWindowClose(Toplevel *tl, const struct wlr_box &curBox) {
    if (!tl) return;
...
void Compositor::animatePopupOpen(Popup *popup) {
...
void Compositor::animatePopupClose(Popup *popup) {
    // No-op for now (popup destroy is immediate)
    (void)popup;
}

void Compositor::animateWorkspaceSwitch(Output *out, int oldWs, int newWs, const struct wlr_box &usable) {
...
void Compositor::applyConfigDecorations() {
...
void Compositor::dumpDebugState() {
    QJsonObject root;
    QJsonArray windows;
