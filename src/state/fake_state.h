...
public:
    ...
    void handle(const VariantEvent& ev) override {
        ...
        for (auto &e : subs) {
            ...
            if (e.mask.window) {
                ...
                if (!win || *win != *e.mask.window) continue;
            }
            e.cb(ev);
        }
    }
    void emitCommand(VariantCommand cmd) override { emitted.push_back(std::move(cmd)); }
    ...
    void unsubscribeOwner(void* owner) override {
        subs.erase(std::remove_if(subs.begin(), subs.end(), [&](auto &e){ return e.owner==owner; }), subs.end());
    }

    void clear() { handled.clear(); emitted.clear(); subs.clear(); }

    std::vector<VariantEvent> handled;
    std::vector<VariantCommand> emitted;

private:
    struct Entry { void* owner; SubMask mask; Callback cb; };
...
public:
    ...
    void addCommandReceiver(CommandReceiver* r) override { receivers.push_back(r); }
    std::vector<VariantCommand> drainCommands() override { auto out = std::move(cmds); cmds.clear(); return out; }
    int nextWakeupMs() const override { return -1; }
    bool needsFrame() const noexcept override { return false; }
    void setNeedsFrame(bool) noexcept override {}
    void emitCommand(VariantCommand c) { cmds.push_back(std::move(c)); }
