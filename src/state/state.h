...
public:
    ...
    Subscription subscribeTyped(void* owner, std::function<void(const T&)> typedCb) {
        ...
        else if constexpr (std::is_same_v<T, AnimTick>) {
            k = EventKind::Anim;
        }
        ...
        return subscribe(
            owner,
            SubMask{uint32_t(k), std::nullopt},
            std::move(wrapper)
        );
    }

    void addEventSource(EventSource* src) override {
        sources.push_back(src);
    }

    void addCommandReceiver(CommandReceiver* r) override {
        receivers.push_back(r);
    }

    void handle(const VariantEvent& ev) override;

    void emitCommand(VariantCommand cmd) override {
        cmdQueue.emplace_back(cmd);
    }
    ...
    const std::vector<VariantCommand>& pendingCommands() const noexcept override {
        return cmdQueue;
    }

    std::vector<VariantCommand>& mutableCommands() noexcept {
        return cmdQueue;
    }
    ...
    struct WindowHandle {
        ...
        Subscription subscribe(void* owner, Callback cb) {
            return state->subscribeWindow(owner, id, std::move(cb));
        }

        std::optional<Box> geometry() const;

        void setBox(Box b, bool isFloating = false) {
            state->emitCommand(Cmd::SetWindowBox{id, b, isFloating});
        }

        void setActivated(bool a) {
            state->emitCommand(Cmd::SetWindowActivated{id, a});
        }

        void setOpacity(float o) {
            state->emitCommand(Cmd::SetWindowOpacity{id, o});
        }
    };

    WindowHandle getWindow(WindowId id) {
        return WindowHandle{id, this};
    }

    std::optional<WindowHandle> findWindow(WindowId id) const {
        if (winMgr.shouldDeliver(id)) {
            return WindowHandle{id, const_cast<State*>(this)};
        }
    ...
    void setNeedsFrame(bool v) noexcept override {
        needsFrameFlag = v;
    }

    void setNextDeadlineMs(std::optional<int> ms) override {
        nextDeadlineMs = ms;
    }
