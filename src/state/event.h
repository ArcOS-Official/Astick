...
inline Box boxFromWlr(int x, int y, int w, int h) noexcept {
    return Box{x, y, w, h};
}
...
constexpr uint32_t kindMask(EventKind k) noexcept {
    return uint32_t(k);
}
...
struct SubMask {
    ...
    static SubMask WindowOf(WindowId id) {
        return {uint32_t(EventKind::Window), id};
    }

    constexpr bool matches(EventKind k, std::optional<WindowId> evWin) const noexcept {
        if ((kinds & uint32_t(k)) == 0) {
            return false;
        }
...
inline SubMask operator|(EventKind k, SubMask m) noexcept {
    m.kinds |= uint32_t(k);
    return m;
}

inline SubMask operator|(SubMask a, EventKind b) noexcept {
    a.kinds |= uint32_t(b);
    return a;
}
...
inline EventKind eventKind(const VariantEvent& ev) noexcept {
    if (std::holds_alternative<KeyEvent>(ev)) {
        return EventKind::Keyboard;
    }

    if (std::holds_alternative<PointerEvent>(ev)) {
        return EventKind::Pointer;
    }

    if (std::holds_alternative<WindowEvent>(ev)) {
        return EventKind::Window;
    }

    if (std::holds_alternative<AnimTick>(ev)) {
        return EventKind::Anim;
    }
