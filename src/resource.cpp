...
void Resource::loadNextIds() {
    ...
    if (!f.open(QIODevice::ReadOnly)) {
        logAndContinue(QStringLiteral("resource.load"), QStringLiteral("loadNextIds failed to open %1: %2").arg(path, f.errorString()));
        return;
    }
    ...
    if (err.error != QJsonParseError::NoError || !doc.isObject()) {
        logAndContinue(QStringLiteral("resource.load"), QStringLiteral("session.json parse error %1").arg(err.errorString()));
        return;
    }
...
void Resource::saveNextIds() {
    ...
    if (!dir.exists()) {
        if (!dir.mkpath(".")) {
            logAndContinue(QStringLiteral("resource.save"), QStringLiteral("saveNextIds mkpath failed %1").arg(dir.path()));
            return;
        }
    }
    QJsonObject obj;
    ...
    if (!f.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
        logAndContinue(QStringLiteral("resource.save"), QStringLiteral("saveNextIds open failed %1: %2").arg(path, f.errorString()));
        return;
    }
...
uint64_t Resource::allocateId(uint64_t base) {
    ...
    if ((counter % ResourceKind::CountPerKind) == 0 && counter != 0) {
        logAndContinue(QStringLiteral("resource.id"), QStringLiteral("ID wrap for base %1 counter %2").arg(base).arg(counter));
    }
    saveNextIds();
    return id;
}
